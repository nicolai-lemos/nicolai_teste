...
$(document).ready(function() {
    ...
    $('#tabela-clientes tr').each(function() {
        ...
        var cpf = $(this).find('td:nth-child(3)').text().toLowerCase().replace(/[^\d]/g, ''); // Remove formatação do CPF para comparação
        var telefone = $(this).find('td:nth-child(4)').text().toLowerCase().replace(/[^\d]/g, ''); // Remove formatação do telefone para comparação

        // Verifica se o input é um número (ID ou CPF ou telefone)
        if (input.match(/^\d+$/)) {
            // Caso seja número, verifica ID, CPF ou telefone
            $(this).toggle(id.startsWith(input) || cpf.startsWith(input) || telefone.startsWith(input));
        } else {
            // Caso seja texto, verifica as duas primeiras letras do nome
            $(this).toggle(nome.startsWith(input));
        }
    });
});
});
